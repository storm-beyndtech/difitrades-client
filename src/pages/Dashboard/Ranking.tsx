...
const Ranking: React.FC = () => {
  ...
  const getCurrentRank = (): RankData => {
    const currentRank = ranks.find((rank) => rank.name === userRank);
    return currentRank || ranks[0]; // fallback to welcome if rank not found
  };
    ...
    <div className="flex items-center justify-center flex-wrap gap-14 p-5 mb-4 rounded-[40px] bg-gray-50 dark:bg-gray-950">
      {ranks.map((rankData) => (
        <RankCard
          key={rankData.level}
          rankData={rankData}
          isCurrentRank={currentRank.name === rankData.name}
